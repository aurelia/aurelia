...
async function generateSummary() {
  ...
  // Friendly intro
  lines.push('This PR was created by the [Aurelia release workflow](https://github.com/aurelia/aurelia/actions/workflows/release.yml). When you\'re ready to release, merge this PR to apply the version bumps, then run the workflow again with `dry_run: false` to publish to npm.');
  lines.push('');
  lines.push('If you\'re not ready yet, that\'s fine — re-running the workflow will update this PR with any new changesets.');
  lines.push('');

  if (preInfo && preInfo.mode === 'pre') {
    const tagLabel = preInfo.tag || 'prerelease';
    lines.push(`> ⚠️ **Pre-release mode** (\`${tagLabel}\`): This will create pre-release versions. To exit pre-release mode, run \`changeset pre exit\` on \`master\` before merging.`);
    lines.push('');
  }
  ...
  lines.push('*Generated by the release workflow.*');
