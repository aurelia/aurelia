...
install dependencies in benchmark folder nmp/yarn
(npm is somehow needed, so it is included in dependencies)
run build script
(it should install all dependencies for subfolders)

run start script (starts http-server that should listen @ 8080)

> Caution:
> Modify package.json in webdriver-ts --chromeBinary should point to the right executable.
> Could try removing it. If it works - great.
```
  "selenium": "cross-env LANG=\"en_US.UTF-8\" node dist/benchmarkRunner.js --chromeBinary \"C:\\Program Files (x86)\\Google\\Chrome Dev\\Application\\chrome.exe\"",
  ```
in another console run bench script
