...
A specific locale can be passed on to format the number using that locale.

The formatting options can be used to affect how the number is being formatted.
A prominent use-case for that is to format the number as currency.
For a full list of options see [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/NumberFormat#Parameters).

Note that both ValueConverter and BindingBehavior updates the formatted value when the active locale changes.
