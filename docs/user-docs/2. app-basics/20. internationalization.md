...
Note that by default the key will be rendered instead of value in case the key is missing in the translated resources.
This is a sensible default as it immediately shows on view that translation is missing for certain keys.
...
Setting `skipTranslationOnMissingKey` to `true` instructs the plugin to return empty string for the missing keys.
...
`@aurelia/i18n` plugin provides relative time formatting service using [`Intl` API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RelativeTimeFormat).
...
Both ValueConverter and BindingBehavior reacts to this signal and updates the view with the formatted value.

#### Relative time format via code

Translating date via code works by using the method `I18nService#rt`.
You can pass in the date as its first parameter, followed by the optional parameters `options`, and `locales`.
...
Additionally, if needed an instance of `Intl.RelativeTimeFormat` can be created using `I18nService#createRelativeTimeFormat` method.
...
```
This can be useful if you want to cache the `Intl.RelativeTimeFormat` instance and reuse that later.
