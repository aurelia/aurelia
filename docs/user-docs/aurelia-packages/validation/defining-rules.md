...
```typescript
validationRules.on(person).ensure("name").required();
```
...
```typescript
validationRules.on(person).ensure("name").matches(/foo/); // name is valid if it contains the string 'foo'
```
...
```typescript
validationRules.on(person).ensure("email").email(); // person's email need to be valid
```
...
```typescript
validationRules.on(person).ensure("name").minLength(42); // name must be at least 42 characters long
```
...
```typescript
validationRules.on(person).ensure("name").maxLength(42); // name must be at most 42 characters long
```
...
```typescript
validationRules.on(person).ensure("pets").minItems(42); // a person should have at least 42 pets
```
...
```typescript
validationRules.on(person).ensure("pets").maxItems(42); // a person should have at most 42 pets
```
...
```typescript
validationRules.on(person).ensure("age").max(42); // a person should be at most 42 years old
```
...
```typescript
validationRules.on(person).ensure("age").range(42, 84); // a person's age should be between 42 and 84 or equal to these values
```
...
```typescript
validationRules.on(person).ensure("age").between(42, 84); // a person's age should be between 42 and 84, but cannot be equal to any these values
```
...
```typescript
validationRules.on(person).ensure("name").equals("John Doe"); // Only people named 'John Doe' are valid
```
...
- `satisfiesRule`
  ...
  class NotTestName extends BaseValidationRule {
    public constructor(private testNames: string[]) {
      super();
    }
...
validationRules.on(person).ensure("name").satisfiesRule(new RequiredRule());
```
...
const customMessages: ICustomMessage[] = [
  ...
  },
  {
    ...
    aliases: [
      {
        name: "min",
        defaultMessage: `\${$displayName} should be at least \${$rule.min}.`,
      },
      {
        name: "max",
        defaultMessage: `\${$displayName} should be at most \${$rule.max}.`,
      },
      {
        name: "range",
        defaultMessage: `\${$displayName} should be between or equal to \${$rule.min} and \${$rule.max}.`,
      },
      {
        name: "between",
        defaultMessage: `\${$displayName} should be between but not equal to \${$rule.min} and \${$rule.max}.`,
      },
    ],
  },
  // ...
...
const customMessages: ICustomMessage[] = [
  {
    ...
    aliases: [
      {
        name: "foobar",
        defaultMessage: `\${$displayName} is required in foo bar.`,
      },
    ],
  },
];
...
```typescript
// ...
validationRules.on(person).ensure("name").required().withMessageKey("foobar");
// ...
```
