...
*   Verify the full property access path, such as `a.b.c`. Initialize intermediate objects or guard the assignment in view-model code, for example `if (a.b != null) a.b.c = value;`. Optional chaining is available for reads and calls; an assignment requires an existing target.
