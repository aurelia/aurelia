...
In the example, shown above, the `Product` component is loaded when the router sees paths like `/product` or `/product/some-id`, that is irrespective of a value for the `id` parameter.
