...
export function $ArraySpeciesCreate(ctx, originalArray, length) {
    const realm = ctx.Realm;
    const intrinsics = realm['[[Intrinsics]]'];
    // 1. Assert: length is an integer Number ≥ 0.
    // 2. If length is -0, set length to +0.
    ...
    return $Construct(ctx, C, new $List(length), intrinsics.undefined);
}
// http://www.ecma-international.org/ecma-262/#sec-createarrayfromlist
export function $CreateArrayFromList(ctx, elements) {
    const realm = ctx.Realm;
    const intrinsics = realm['[[Intrinsics]]'];
