...
// === SOURCE ===
// HTML:
// <template><div with.bind="obj['key']">${deep.v}<button click.trigger="doThing(1, 'ok')"></button></div></template>
//
// CLASSES:
export {}
class Common {
...
function __au$access(_fn) { /* no-op */ }function __typecheck_template_Common__() {
  ...
  return `<template><div with.bind="${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.deep.v, "obj['key']")}">${access((/** @type {__AU_TTC_T0_F0} */(o)) => o.obj["key"], "deep.v")}<button click.trigger="${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.doThing(1, "ok"), "doThing(1, 'ok')")}"></button></div></template>`;
}
