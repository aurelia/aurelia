...
function __au$access(_fn) { /* no-op */ }function __typecheck_template_Common__() {
  ...
  return `<template><div promise.bind="${access((/** @type {__AU_TTC_T0_F2} */(o)) => o.r.users[0].email, "data")}"><template then="${access((_o) => void 0, "r")}">${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.data, "r.users[(0)].email")}</template></div></template>`;
}
