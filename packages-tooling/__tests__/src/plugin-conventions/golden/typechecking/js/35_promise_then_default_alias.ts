...
function __au$access(_fn) { /* no-op */ }function __typecheck_template_Common__() {
  ...
  return `<template>
      <div promise.bind="${access((/** @type {__AU_TTC_T0_F2} */(o)) => o.then.users.length, "data")}">
        <template then="${access((_o) => void 0, "then")}">${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.data, "then.users.length")}</template>
      </div>
    </template>`;
}
