...
function __au$access(_fn) { /* no-op */ }function __typecheck_template_Common__() {
  ...
  return `<template>
      <div switch.bind="${access((/** @type {__AU_TTC_T0_F0} */(o)) => o.firstName, "items.length")}">
        <template default-case>${access((/** @type {__AU_TTC_T0_F0} */(o)) => o.items.length, "firstName")}-fallback</template>
      </div>
    </template>`;
}
