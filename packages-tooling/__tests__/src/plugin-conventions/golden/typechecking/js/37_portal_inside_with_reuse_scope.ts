...
function __au$access(_fn) { /* no-op */ }function __typecheck_template_Common__() {
  ...
  return `<template>
      <div with.bind="${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.$parent.firstName, "deep")}">
        <template portal>${access((/** @type {__AU_TTC_T0_F0} */(o)) => o.deep, "$parent.firstName")}:${access((/** @type {__AU_TTC_T0_F1} */(o)) => o.portal, "v")}</template>
      </div>
    </template>`;
}
