...
type __AU_TTC_T0_F0 = (Common) & { $parent: unknown } & { deep: (Common)['obj']['key']['deep'] };
function __typecheck_template_Common__(): string {
  const access = <T extends object>(typecheck: (o: T) => unknown, expr: string): string => expr;
  return `<template><let deep.bind="${access<__AU_TTC_T0_F0>(o => o.deep.v, "obj['key'].deep")}"></let>${access<__AU_TTC_T0_F0>(o => o.obj["key"].deep, "deep.v")}</template>`;
}
