...
function __typecheck_template_Common__(): string {
  ...
  return `<template><div promise.bind="${access<__AU_TTC_T0_F2>(o => o.r.users[0].email, "data")}"><template then="${access((_o) => void 0, "r")}">${access<__AU_TTC_T0_F1>(o => o.data, "r.users[(0)].email")}</template></div></template>`;
}
