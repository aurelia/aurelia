/* AUTO-GENERATED: 37_portal_inside_with_reuse_scope
...
type __AU_TTC_T0_F1 = (Common)['deep'] & { $this: (Common)['deep'] } & (Common) & { $parent: unknown };
function __typecheck_template_Common__(): string {
  const access = <T extends object>(typecheck: (o: T) => unknown, expr: string): string => expr;
  return `<template>
      <div with.bind="${access<__AU_TTC_T0_F1>(o => o.$parent.firstName, "deep")}">
        <template portal>${access<__AU_TTC_T0_F0>(o => o.deep, "$parent.firstName")}:${access<__AU_TTC_T0_F1>(o => o.portal, "v")}</template>
      </div>
    </template>`;
}
