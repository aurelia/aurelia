import { camelCase, kebabCase, isArray, DI } from '@aurelia/kernel';
import * as path from 'path';
import pkg from 'typescript';
import * as $modifyCode from 'modify-code';
import $modifyCode__default from 'modify-code';
import { AccessScopeExpression, Unparser, AccessMemberExpression, ExpressionParser } from '@aurelia/expression-parser';
import { DotSeparatedAttributePattern, EventAttributePattern, RefAttributePattern, AtPrefixedTriggerAttributePattern, ColonPrefixedBindAttributePattern, IAttributeParser } from '@aurelia/template-compiler';
...
function __typecheck_template_${classes.map(x => x.name).join('_')}__() {
  ...
  return \`${html}\`;
        ...
        if (isArray(args)) {
            ...
            switch (len) {
                ...
                case 1: {
                    ...
                    return `${_a._o} => ${_a._o}.${arg0}`;
                }
                default: {
                    const parts = args.map(arg => arg.startsWith('this.') ? arg.slice(5) : arg);
                    return `${_a._o} => (${parts.map(arg => `${_a._o}.${arg}`).join(',')})`;
                }
            }
        }
        switch (args.$kind) {
            ...
            case 'ObjectLiteral':
            case 'ArrayLiteral':
                return `${_a._o} => (${Unparser.unparse(args)})`;
            case 'CallScope':
                {
                    const argList = [];
    ...
    createMemberAccessExpression(member) {
        return new AccessMemberExpression(_a.rootAccessScope, member);
    }
    pushScope(type) {
...
TypeCheckingContext.rootAccessScope = new AccessScopeExpression(_a._o, 0);
function createTypeCheckedTemplate(rawHtml, classes, isJs) {
    const tree = parseFragment(rawHtml, { sourceCodeLocationInfo: true });
    const container = DI.createContainer().register(DotSeparatedAttributePattern, EventAttributePattern, RefAttributePattern, AtPrefixedTriggerAttributePattern, ColonPrefixedBindAttributePattern);
    ...
    traverse$1(tree, (node) => processNode(node, ctx), ctx);
    return ctx.produceTypeCheckedTemplate(rawHtml);
}
function traverseDepth($node, ctx) {
    var _b;
    if ($node.childNodes)
        traverse$1($node, n => processNode(n, ctx), ctx);
    if ((_b = $node.content) === null || _b === void 0 ? void 0 : _b.childNodes)
        traverse$1($node.content, n => processNode(n, ctx), ctx);
    ctx.popScope();
}
function tryProcessRepeat(syntax, attr, node, ctx) {
    ...
    if (expr.iterable.$kind === 'PrimitiveLiteral') {
        ...
        const [iterIdent, path] = mutateAccessScope(expr.iterable, ctx, member => { var _b; return (_b = ctx.getIdentifier(member, 1)) !== null && _b !== void 0 ? _b : ctx.getIdentifier(member); }, true);
        if (path.length > 0) {
...
function tryProcessWith(syntax, attr, node, ctx) {
    var _b;
    if (syntax.target !== 'with')
        return [false, void 0];
    ctx.pushScope('none');
    const value = attr.value.length === 0 ? syntax.target : attr.value;
    const expr = ctx.exprParser.parse(value, 'None');
    if (expr != null) {
        const [rewritten, path] = mutateAccessScope(expr, ctx, m => { var _b; return (_b = ctx.getIdentifier(m, 1)) !== null && _b !== void 0 ? _b : m; }, true);
        ctx.toReplace.push({
            loc: node.sourceCodeLocation.attrs[attr.name],
            modifiedContent: () => `${attr.name}="\${${ctx.accessIdentifier}(${ctx.createLambdaExpression(rewritten)}, '${escape(value)}')}"`
        });
        const baseExpr = (path === null || path === void 0 ? void 0 : path.length) ? `${ctx.accessTypeIdentifier}${path.map(p => `['${p}']`).join('')}` : null;
        const first = (_b = path === null || path === void 0 ? void 0 : path[0]) !== null && _b !== void 0 ? _b : null;
        const env = baseExpr == null ? null : { mode: (first === null || first === void 0 ? void 0 : first.startsWith(identifierPrefix)) ? 'synthetic' : 'vm', expr: baseExpr };
        if (env != null)
            ctx.pushEnv(env);
        traverseDepth(node, ctx);
        if (env != null)
...
function makeEnvResolver(ctx) {
    return (member) => {
        ...
        return (_b = ctx.getIdentifier(member, 1)) !== null && _b !== void 0 ? _b : member;
    };
}
function computeIndexBase(env, ctx) {
...
const allowedLetBindingCommands = ['bind', 'one-time', 'to-view', 'two-way'];
    ...
    if ('tagName' in node) {
        if (node.tagName === 'let') {
            ...
            for (const attr of node.attrs) {
                ...
                if (!command || !allowedLetBindingCommands.includes(command))
                    continue;
                const value = attr.value.length === 0 ? target : attr.value;
                const ast = ctx.exprParser.parse(value, 'None');
                if (ast == null)
                    continue;
                const [_expr, path] = mutateAccessScope(ast, ctx, m => { var _b; return (_b = ctx.getIdentifier(m, 1)) !== null && _b !== void 0 ? _b : m; }, true);
                ...
                if (isPrimitive) {
                    inferredType = Unparser.unparse(_expr);
                }
                else if (path === null || path === void 0 ? void 0 : path.length) {
                    inferredType = `${ctx.accessTypeIdentifier}${path.map(p => `['${p}']`).join('')}`;
                }
                const propName = hasToBindingContext ? target : ctx.getIdentifier(target, 2);
...
function traverse$1(tree, cb, ctx) {
    ctx.pushScope('none');
