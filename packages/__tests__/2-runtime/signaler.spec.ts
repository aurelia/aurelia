    ...
    ([$11, $12, mocks], [$21, signals], [$31, flags]) => [`dispatchSignal             `, sut => {
      for (let i = 0, ii = signals.length; i < ii; ++i) {
        ...
        for (let j = 0, jj = mocks.length; j < jj; ++j) {
          ...
          assert.strictEqual(mock.calls[i][3], flags | LifecycleFlags.updateTargetInstance, `mock.calls[i][3]`);
        }
      }
    }],
    ([$11, $12, mocks], [$21, signals], [$31, flags]) => [`dispatchSignal x2          `, sut => {
      for (let i = 0, ii = signals.length; i < ii; ++i) {
        ...
        for (let j = 0, jj = mocks.length; j < jj; ++j) {
          ...
          assert.strictEqual(mock.calls[i * 2 + 1][3], flags | LifecycleFlags.updateTargetInstance, `mock.calls[i * 2 + 1][3]`);
        }
      }
    }]
  ];
