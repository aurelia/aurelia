  ...
  const testCases: IHarmoniousCompilationTestCase[] = [
    ...
    },
    {
      ...
      assertFn: (ctx, host) => {
        ...
        const $ceViewModel = CustomElement.behaviorFor(ceEl).viewModel as { hasFocus: boolean; focus: number; blur: number };
    ...
    ...Array.from({ length: 10 }, (_, idx) => {
      return {
        ...
        assertFn: (ctx, host) => {
          ...
          const $leafCeVm = CustomElement.behaviorFor(leafCeHost).viewModel as { focus: number };
