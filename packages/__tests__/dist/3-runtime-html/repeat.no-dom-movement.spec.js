...
describe('3-runtime-html/repeat.no-dom-movement.spec.ts', function () {
    it('[non-keyed] does not remove existing node when adding a second item', async function () {
        const { appHost, component } = createFixture(`<div repeat.for="item of items">\${item}</div>`, class {
            constructor() {
                this.items = [0];
            }
        });
    ...
    it('[keyed] does not remove existing node when adding a second item ', async function () {
        const { appHost, component } = createFixture(`<div repeat.for="item of items; key: id">\${item.id}</div>`, class {
            constructor() {
                this.items = [{ id: 0 }];
            }
        });
    ...
    it('does not lose focus when adding a second item', async function () {
        const { appHost, component } = createFixture(`<input repeat.for="item of items" value.bind="item" />`, class {
            constructor() {
                this.items = ['first'];
            }
        });
        ...
        const firstInput = appHost.querySelector('input');
    ...
    it('properly detects DOM operations using detailed MutationObserver tracking', async function () {
        const { appHost, component } = createFixture(`<div repeat.for="item of items" id="\${item}">\${item}</div>`, class {
            constructor() {
                this.items = [0];
            }
        });
        ...
        assert.strictEqual(unexpectedMutations.length, 0, `Unexpected mutations for first item: ${unexpectedMutations.join(', ')}`);
    ...
    it('tracks actual DOM removal with disconnected callback', async function () {
        ...
        const { appHost, component } = createFixture(`<div repeat.for="item of items" custom-attr>\${item}</div>`, class {
            constructor() {
                this.items = [0];
            }
        }, [
    ...
    it('does not deactivate and reactivate existing views when adding items', async function () {
        ...
        const { component } = createFixture(`<div repeat.for="item of items"><span lifecycle-tracker.bind="'item: ' + item">\${item}</span></div>`, class {
            constructor() {
                this.items = [0];
            }
        }, [
            (_a = class LifecycleTracker {
                    binding() {
        ...
        assert.strictEqual(firstItemEvents.length, 0, `First item should not have lifecycle events. Got: ${firstItemEvents.join(', ')}`);
...
//# sourceMappingURL=repeat.no-dom-movement.spec.js.map