...
function getViewports(transitions) {
    const viewports = [];
    let topViewport;
...
export function getNonSiblingHooks(deferUntil, swapStrategy, phase, transitionComponents) {
    ...
    const { viewports, topViewport, addViewports, removeViewports, underlyingAdd, topFrom, topTo, } = getViewports(transitions /* swapStrategy.includes('parallel') && deferUntil !== 'none' */);
