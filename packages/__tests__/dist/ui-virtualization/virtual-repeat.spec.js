...
import { DefaultVirtualizationConfiguration, VirtualRepeat, VIRTUAL_REPEAT_NEAR_BOTTOM, VIRTUAL_REPEAT_NEAR_TOP } from '@aurelia/ui-virtualization';
...
describe('ui-virtualization/virtual-repeat.spec.ts', function () {
    ...
    it('rerenders when scrolled', function () {
        const { scrollBy } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item.name}</div>'), class App {
            constructor() {
                this.items = createItems();
            }
        }, virtualRepeatDeps);
    ...
    describe('scroller resizing', function () {
        it('works with dynamic height scroller', async function () {
            const { getAllBy } = createFixture(`<main style="max-height: 300px; width: 300px; overflow: auto; background-color: lightgray;">
        ...
        it('works with container starting with 0 height', async function () {
            const { getAllBy, getBy } = createFixture(`<main style="height: 0px; width: 300px; overflow: auto; background-color: lightgray;">
          <div virtual-repeat.for="item of items" style="height: 50px">\${item.name}</div>
        </main>`, class App {
                constructor() {
                    this.items = createItems();
    ...
    describe('mutation', function () {
        ...
        it('rerenders when removed at the start', async function () {
            const { component } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items" style="height: 50px">${item.name}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
    ...
    describe('configuration', function () {
        it('accepts item-height configuration', function () {
            createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; item-height: 30" style="height: 30px">${item}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
        ...
        it('accepts buffer-size configuration', function () {
            createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; buffer-size: 4" style="height: 50px">${item}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
        ...
        it('accepts multiple configuration values', function () {
            createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; item-height: 40; buffer-size: 3; min-views: 5" style="height: 40px">${item}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
        ...
        it('accepts horizontal layout with item-width configuration', function () {
            createFixture(createHorizontalScrollerTemplate('<div virtual-repeat.for="item of items; layout: horizontal; item-width: 100" style="width: 100px; height: 50px; display: inline-block">${item}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
    ...
    describe('horizontal scrolling', function () {
        it('renders horizontally with layout configuration', function () {
            createFixture(createHorizontalScrollerTemplate('<div virtual-repeat.for="item of items; layout: horizontal; item-width: 80" style="width: 80px; height: 50px; display: inline-block">${item.name}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
        ...
        it('rerenders when scrolled horizontally', function () {
            const { scrollBy } = createFixture(createHorizontalScrollerTemplate('<div virtual-repeat.for="item of items; layout: horizontal; item-width: 80" style="width: 80px; height: 50px; display: inline-block">${item.name}</div>'), class App {
                constructor() {
                    this.items = createItems();
                }
            }, virtualRepeatDeps);
    ...
    describe('infinite scroll', function () {
        it('triggers near-bottom event when scrolling to the end', function (done) {
            const { scrollBy } = createFixture(`<div
          id="scroller"
    ...
    describe('variable sizing', function () {
        it('renders with variable height enabled', function () {
            createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; variable-height: true" style="height: ${item.height}px">${item.name}</div>'), class App {
                constructor() {
                    this.items = createVariableItems();
                }
            }, virtualRepeatDeps);
        ...
        it('renders with variable width enabled in horizontal layout', function () {
            createFixture(createHorizontalScrollerTemplate('<div virtual-repeat.for="item of items; layout: horizontal; variable-width: true" style="width: ${item.width}px; height: 50px; display: inline-block">${item.name}</div>'), class App {
                constructor() {
                    this.items = createVariableItems();
                }
            }, virtualRepeatDeps);
        ...
        it('scrolls correctly with variable height', function () {
            const { scrollBy } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; variable-height: true" style="height: ${item.height}px">${item.name}</div>'), class App {
                constructor() {
                    this.items = createVariableItems();
                }
            }, virtualRepeatDeps);
        ...
        it('scrolls correctly with variable width in horizontal layout', function () {
            const { scrollBy } = createFixture(createHorizontalScrollerTemplate('<div virtual-repeat.for="item of items; layout: horizontal; variable-width: true" style="width: ${item.width}px; height: 50px; display: inline-block">${item.name}</div>'), class App {
                constructor() {
                    this.items = createVariableItems();
                }
            }, virtualRepeatDeps);
        ...
        it('handles mutation with variable height', function () {
            const { component } = createFixture(createScrollerTemplate('<div virtual-repeat.for="item of items; variable-height: true" style="height: ${item.height}px">${item.name}</div>'), class App {
                constructor() {
                    this.items = createVariableItems();
                }
            }, virtualRepeatDeps);
    ...
    function createHorizontalScrollerTemplate(content, styles) {
        let { width = 600, height = 100, padding = 0, border = 0, 
        // eslint-disable-next-line prefer-const
        boxSizing = 'border-box', } = styles ?? {};
