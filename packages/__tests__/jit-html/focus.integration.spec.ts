...
import { eachCartesianJoin, HTMLTestContext, TestContext, assert } from '@aurelia/testing';

describe.only('focus.integration.spec.ts', function() {
  ...
  describe('Interactive scenarios', function() {
    ...
    eachCartesianJoin(
      ...
      (command, { title, template, getFocusable, app, assertionFn }: ITestCase) => {
        it(title, function() {
          const { au, component, ctx } = setupAndStartNormal<IApp>(
            template(command),
            app
