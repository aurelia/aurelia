      ...
      describe('with custom element', function () {
        ...
        describe('compiles surrogate', function () {
          ...
          it('compiles surrogate plain class attribute', function () {
            ...
            verifyInstructions(
              surrogates,
              [{ toVerify: ['type', 'value'], type: itSetClassAttribute, value: 'h-100' }]
            );
          });
          ...
          it('compiles surrogate plain style attribute', function () {
            ...
            verifyInstructions(
              surrogates,
              [{ toVerify: ['type', 'value'], type: itSetStyleAttribute, value: 'background: red' }]
            );
          });
          ...
          it('compiles surrogate with binding expression', function () {
            ...
            verifyInstructions(
              surrogates,
              [{ toVerify: ['type', 'to'], type: itPropertyBinding, to: 'class' }],
              'surrogate'
            );
          });
          ...
          it('compiles surrogate with interpolation expression', function () {
            ...
            verifyInstructions(
              surrogates,
              [{ toVerify: ['type', 'to'], type: itInterpolation, to: 'class' }],
              'surrogate'
            );
          });
          ...
          it('does not create a prop binding when attribute value is an empty string', function () {
            ...
            verifyInstructions(surrogates, [
              { toVerify: ['type', 'to', 'res', 'props'], type: itHydrateAttribute, res: 'foo', props: [] }
            ], 'surrogate');
          });
          ...
          it('compiles surrogate with interpolation binding + custom attribute', function () {
            ...
            verifyInstructions(
              surrogates,
              [
                { toVerify: ['type', 'to', 'props'], type: itHydrateAttribute, res: 'foo', props: [
                  { type: itInterpolation, from: createInterpolation(['', ''], [createAccessScopeExpression('bar')]), to: 'value' }
                ]}
              ],
              'surrogate'
            );
        ...
        it('understands attr precendence: element prop > custom attr', function () {
          ...
          const rootInstructions = actual.instructions[0][0]['props'] as readonly IInstruction[];
          const expectedRootInstructions = [
            { toVerify: ['type', 'res', 'to'], type: itPropertyBinding, to: 'prop1' },
            { toVerify: ['type', 'res', 'to'], type: itPropertyBinding, to: 'prop2' },
            { toVerify: ['type', 'res', 'to'], type: itPropertyBinding, to: 'prop3' },
            { toVerify: ['type', 'res', 'to'], type: itSetProperty, to: 'prop3' }
          ];
        ...
        it('distinguishes element properties / normal attributes', function () {
          ...
          const expectedRootInstructions = [
            { toVerify: ['type', 'res'], type: itHydrateElement, res: 'el' }
          ];
          verifyInstructions(rootInstructions, expectedRootInstructions);

          const expectedElInstructions = [
            { toVerify: ['type', 'to', 'value'], type: itSetProperty, to: 'name', value: 'name' }
          ];
        ...
        it('understands element property casing', function () {
          ...
          const expectedElInstructions = [
            { toVerify: ['type', 'value', 'to'], type: itSetProperty, value: 'label', to: 'backgroundColor' },
          ];
          verifyInstructions((rootInstructions[0] as HydrateElementInstruction).props, expectedElInstructions);
        });
            ...
            verifyInstructions(hydratePropAttrInstruction.props, [
              {
                ...
                type: itPropertyBinding, to: 'value', from: createAccessScopeExpression('p')
              }
            ]);
            verifyInstructions(hydratePropAttrInstruction.def.instructions[0], [
              {
                toVerify: ['type', 'to', 'from'],
                type: itPropertyBinding, to: 'name', from: createAccessScopeExpression('name')
              },
              {
                toVerify: ['type', 'to', 'from'],
                type: itPropertyBinding, to: 'title', from: createAccessScopeExpression('title')
              },
            ]);
          });
          ...
          describe('[as-element]', function () {
            it('understands [as-element]', function () {
              ...
              verifyInstructions(instructions[0], [
                {
                  toVerify: ['type', 'res'],
                  type: itHydrateElement, res: 'not-div'
                }
              ]);
            });
            ...
            describe('with template controller', function () {
              it('compiles', function () {
                ...
                verifyInstructions(instructions[0], [
                  {
                    ...
                    type: itHydrateTemplateController, res: 'if'
                  }
                ]);
                ...
                verifyInstructions(templateControllerInst.props, [
                  {
                    toVerify: ['type', 'to', 'from'],
                    type: itPropertyBinding, to: 'value', from: createAccessScopeExpression('value')
                  }
                ]);
        ...
        describe('<let/> element', function () {
          ...
          it('ignores custom element resource', function () {
            ...
            verifyInstructions(instructions[0], [
              { toVerify: ['type'], type: itHydrateLetElement }
            ]);
          });
            ...
            verifyInstructions((instructions[0][0] as HydrateLetElementInstruction).instructions, [
              {
                ...
                type: itLetBinding, to: 'a', from: 'b'
              },
              {
                toVerify: ['type', 'to'],
                type: itLetBinding, to: 'c'
              }
            ]);
          });
          ...
          describe('[to-binding-context]', function () {
            ...
            it('ignores [to-binding-context] order', function () {
              ...
              verifyInstructions(instructions, [
                { toVerify: ['type', 'toBindingContext'], type: itHydrateLetElement, toBindingContext: true }
              ]);
        ...
        describe('with containerless', function () {
          ...
          it('compiles [containerless] next to each other', function () {
            ...
            assertTemplateHtml(template, '<au-m></au-m><!--au-start--><!--au-end--><au-m></au-m><!--au-start--><!--au-end-->');
          });
        });
        ...
        describe('with higher number hids (stress tests)', function () {
          it('compiles 10 text interpolations with sequential hids', function () {
            ...
            // Each interpolation gets a marker: <au-m></au-m> followed by space
          ...
          it('compiles many elements with bindings (implicit ordering)', function () {
            ...
            // Each element gets an <au-m> marker before it
            assertTemplateHtml(template,
              '<au-m></au-m><div></div><au-m></au-m><div></div><au-m></au-m><div></div>' +
              '<au-m></au-m><div></div><au-m></au-m><div></div><au-m></au-m><div></div>'
            );
          ...
          it('compiles mixed interpolations and element bindings (implicit ordering)', function () {
            ...
            assertTemplateHtml(template,
              '<au-m></au-m> <au-m></au-m><div></div><au-m></au-m> <au-m></au-m><span></span><au-m></au-m> '
            );
          ...
          it('compiles containerless elements interspersed with regular bindings', function () {
            ...
            // All targets get <au-m> markers, containerless also get render location
            assertTemplateHtml(template,
              '<au-m></au-m><div></div><au-m></au-m><!--au-start--><!--au-end-->' +
              '<au-m></au-m><div></div><au-m></au-m><!--au-start--><!--au-end-->'
            );
          ...
          it('compiles 15 consecutive containerless elements', function () {
            ...
            const expected = Array(15).fill('<au-m></au-m><!--au-start--><!--au-end-->').join('');
        ...
        describe('with deeply nested hids', function () {
          it('compiles template controller with nested element binding (child template has fresh hid counter)', function () {
            ...
            assertTemplateHtml(tcInstruction.def.template!, '<au-m></au-m><div></div>');
          ...
          it('compiles triple-nested template controllers with element at leaf', function () {
            ...
            assertTemplateHtml(ifTC.def.template!, '<au-m></au-m><!--au-start--><!--au-end-->');

            // Level 2: for controller
            const forTC = ifTC.def.instructions![0][0] as HydrateTemplateController;
            assertTemplateHtml(forTC.def.template!, '<au-m></au-m><div></div>');
          });
          ...
          it('compiles nested template controller with multiple bindings at leaf', function () {
            ...
            const tcInstruction = instructions[0][0] as HydrateTemplateController;
            assertTemplateHtml(tcInstruction.def.template!, '<div><au-m></au-m><span></span></div>');
          ...
          it('compiles complex nested structure with mixed marker types', function () {
            ...
            assertTemplateHtml(template, '<au-m></au-m><div><au-m></au-m> <au-m></au-m><!--au-start--><!--au-end--></div>');
            ...
            assertTemplateHtml(ifTC.def.template!, '<au-m></au-m><span></span>');
          });
          ...
          it('compiles 5-level deep nesting with bindings at each level', function () {
            ...
            for (let level = 0; level < 4; level++) {
              ...
              assertTemplateHtml(tc.def.template!, '<au-m></au-m><!--au-start--><!--au-end-->');
            ...
            assertTemplateHtml(lastTC.def.template!, '<au-m></au-m><div></div>');
          ...
          it('compiles sibling template controllers with independent hid sequences', function () {
            ...
            for (let i = 0; i < 3; i++) {
              ...
              assertTemplateHtml(tc.def.template!, '<au-m></au-m><div></div>');
            }
          });
        });
      });
  ...
  describe(`combination assertions`, function () {
      ...
      for (const debug of [true, false]) {
        it(`[debug: ${debug}] compiles ref`, function () {
          ...
          verifyBindingInstructionsEqual(result, {
            ...
            needsCompile: false,
            template: debug
              ? '<template><au-m></au-m><div ref="el"></div></template>'
              : '<template><au-m></au-m><div></div></template>',
        ...
        it(`[debug: ${debug}] compiles data-* attributes`, function () {
          ...
          verifyBindingInstructionsEqual(result, {
            name: 'unamed',
            template: debug
              ? '<template><au-m></au-m><div data-a="b" data-b.bind="1" data-c="${hey}"></div></template>'
              : '<template><au-m></au-m><div data-a="b"></div></template>',
            needsCompile: false,
            instructions: [[
        ...
        it(`[debug: ${debug}] compiles class attribute`, function () {
          ...
          verifyBindingInstructionsEqual(result, {
            name: 'unamed',
            template: debug
              ? '<template><au-m></au-m><div d.class="a" class="a ${b} c"></div></template>'
              : '<template><au-m></au-m><div></div></template>',
            needsCompile: false,
            instructions: [[
        ...
        it(`[debug: ${debug}] compiles style attribute`, function () {
          ...
          verifyBindingInstructionsEqual(result, {
            name: 'unamed',
            template: debug
              ? '<template><au-m></au-m><div bg.style="a" style="a ${b} c"></div></template>'
              : '<template><au-m></au-m><div></div></template>',
            needsCompile: false,
            instructions: [[
    ...
    describe('TemplateCompiler - combinations -- custom attributes', function () {
      ...
      it('compiles custom attribute without value', function () {
        ...
        verifyBindingInstructionsEqual(result, {
          name: 'unamed',
          template: '<template><au-m></au-m><div></div></template>',
          needsCompile: false,
          instructions: [[{
      ...
      it('compiles attribute on a template element', function () {
        ...
        verifyBindingInstructionsEqual(result, {
          name: 'unamed',
          template: '<template><au-m></au-m><template></template></template>',
          needsCompile: false,
          instructions: [[{
            type: itHydrateAttribute,
            res: CustomAttribute.getDefinition(MyAttr),
    ...
    describe('TemplateCompiler - combinations -- custom attributes with multiple bindings', function () {
      ...
      it('compiles an attribute with 2 bindings', function () {
        ...
        verifyBindingInstructionsEqual(result.instructions[0][0], {
          type: itHydrateAttribute,
          res: CustomAttribute.getDefinition(MyAttr),
          props: [createProp({ from: '1', to: 'a' }), { type: itSetProperty, value: '2', to: 'b' }]
        });
      });
      ...
      it('compiles multiple attributes with 2 bindings', function () {
        ...
        verifyBindingInstructionsEqual(result.instructions[0][1], {
          ...
          props: [{ type: itSetProperty, value: '3', to: 'c' }, createProp({ from: '4', to: 'd', mode: BindingMode.oneTime })]
        });
      });
    ...
    describe('TemplateCompiler - combinations -- nested template controllers (one per element)', function () {
      ...
      it('compiles nested template controller', function () {
        ...
        verifyBindingInstructionsEqual(result, {
          name: 'unamed',
          needsCompile: false,
          template: '<template><au-m></au-m><!--au-start--><!--au-end--></template>',
          dependencies: [],
          surrogates: [],
          hasSlots: false,
          type: 'custom-element',
          instructions: [[{
            ...
            def: {
              name: 'unamed',
              needsCompile: false,
              template: '<template><div><au-m></au-m><!--au-start--><!--au-end--></div></template>',
              instructions: [[{
                type: itHydrateTemplateController,
                ...
                def: {
                  name: 'unamed',
                  needsCompile: false,
                  template: '<template><div id="2"><au-m></au-m><!--au-start--><!--au-end--></div></template>',
                  type: 'custom-element',
                  instructions: [[{
                    type: itHydrateTemplateController,
                    ...
                    def: {
                      name: 'unamed',
                      needsCompile: false,
                      template: '<template><div id="3"><au-m></au-m><!--au-start--><!--au-end--></div></template>',
                      type: 'custom-element',
                      instructions: [[{
                        type: itHydrateTemplateController,
    ...
    describe('TemplateCompiler - combinations -- attributes on custom elements', function () {
      ...
      it('compiles a custom attribute on a custom element', function () {
        ...
        verifyBindingInstructionsEqual(result, {
          name: 'unamed',
          template: '<template><au-m></au-m><my-el foo="bar"></my-el></template>',
          needsCompile: false,
          instructions: [[
            createElement({
              ctor: MyEl
    ...
    describe('TemplateCompiler - combinations -- captures & ...$attrs & ...', function () {
      ...
      it('compiles shorthand spread syntax', function () {
        ...
        verifyBindingInstructionsEqual(definition.instructions[0], [
          { type: itHydrateElement, res: 'my-element', props: [{ type: itSpreadValueBinding, target: '$bindables', from: 'item' }], projections: null, containerless: false, captures: [], data: {} },
        ]);
      });
    ...
    describe('TemplateCompiler - combinations -- with attribute patterns', function () {
      ...
      it('works with pattern returning command', function () {
        ...
        assert.deepStrictEqual(
          ...
          [{ type: itPropertyBinding, from: createAccessScopeExpression('id'), to: 'id', mode: BindingMode.toView }]
        );
      });
      ...
      it('works when pattern returning interpolation', function () {
        ...
        assert.deepStrictEqual(
          result.instructions[0],
          [{ type: itInterpolation, from: createInterpolation(['', 'a'], [createAccessScopeExpression('a')]), to: 'id' }]
        );
      });
      ...
      it('lets pattern control the binding value', function () {
        ...
        assert.deepStrictEqual(
          ...
          [{ type: itPropertyBinding, from: createAccessScopeExpression('bb'), to: 'id', mode: BindingMode.toView }]
        );
      });
      ...
      it('works with pattern returning custom attribute + interpolation', function () {
        ...
        assert.deepStrictEqual(
          result.instructions[0],
          [{ type: itHydrateAttribute, res: CustomAttribute.getDefinition(MyAttr), alias: undefined, props: [
            { type: itInterpolation, from: createInterpolation(['', ''], [createAccessScopeExpression('bb')]), to: 'value' }
          ]}]
        );
      });
    });
  ...
  function compileTemplate(markupOrOptions: string | Element | { template: string; debug?: boolean; resolveResources?: boolean }, ...extraResources: any[]) {
    ...
    return {
      ...
      createElement: ({ ctor, props = [], projections = null, containerless = false, captures = [], data = {} }: {
        ...
        ({ type: itHydrateElement, res: CustomElement.getDefinition(ctor), props, projections: projections as Record<string, IElementComponentDefinition>, containerless, captures, data }),
      createSetProp: ({ value, to }: { value: unknown; to: string }): SetPropertyInstruction =>
        ({ type: itSetProperty, value, to }),
      createRef: (name: string, to: string): RefBindingInstruction => ({ type: itRefBinding, from: parser.parse(name, 'IsProperty'), to }),
      createPropBinding: ({ from, to, mode = BindingMode.toView }: { from: string; to: string; mode?: BindingMode }): PropertyBindingInstruction =>
        ({ type: itPropertyBinding, from: parser.parse(from, 'IsFunction'), to, mode }),
      createAttr: ({ attr, from, to }: { attr: string; from: string; to: string }): AttributeBindingInstruction =>
        ({ type: itAttributeBinding, attr, from: parser.parse(from, 'IsProperty'), to }),
      createInterpolation: ({ from, to }: { from: string; to: string }): InterpolationInstruction =>
        ({ type: itInterpolation, from: parser.parse(from, 'Interpolation'), to }),
      createIterateProp: (expression: string, to: string, props: any[]): IteratorBindingInstruction =>
        ({ type: itIteratorBinding, forOf: parser.parse(expression, 'IsIterator'), to, props })
    };
  }
});
