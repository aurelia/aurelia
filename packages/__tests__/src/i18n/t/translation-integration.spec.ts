...
describe('i18n/t/translation-integration.spec.ts', function () {
  ...
  it('different aliases can be used for different apps', async function () {
    ...
    async function createAppOne() {
      ...
      return { au, host, vm: au.root.controller.viewModel as AppOne, queue: ctx.platform.taskQueue };
    }
    ...
    async function createAppTwo() {
      ...
      await au.start();
      return { au, host, vm: au.root.controller.viewModel as AppTwo, queue: ctx.platform.taskQueue };
    }
