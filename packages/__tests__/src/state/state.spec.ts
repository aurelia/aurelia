...
describe('state/state.spec.ts', function () {
  ...
  describe('multiple stores', function () {
    it('binds to named stores via store helper', async function () {
      const { getBy } = await createFixture
        .html`
          <span id="user" textcontent.state="name & store:'users'"></span>
          <span id="product" textcontent.state="title & store:'products'"></span>
        `
        .deps(
    ...
    it('dispatches actions to the targeted store', async function () {
      ...
      const { getBy, ctx } = await createFixture
        .html`
          ...
          <button id="users" click.dispatch="{ type: 'increment' } & store:'users'">users</button>
        `
        .deps(
