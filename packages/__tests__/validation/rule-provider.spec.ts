...
describe('ValidationMessageProvider', function () {
  ...
  for (let i = 0, ii = rules.length; i < ii; i++) {
    ...
    it(`rule.message returns the registered message for a rule instance - ${title}`, function () {
      ...
      const actual = sut.getMessage($rule).evaluate(LifecycleFlags.none, scope, null, container);
  ...
  it('appending new custom key and messages is also possible', function () {
    ...
    const actual1 = sut.getMessage($rule1).evaluate(LifecycleFlags.none, scope1, null, container);
    const actual2 = sut.getMessage($rule2).evaluate(LifecycleFlags.none, scope2, null, container);
