...
export function $$arrayBindingElement(node, parent, ctx, idx) {
    switch (node.kind) {
...
export function $$arrayBindingElementList(nodes, parent, ctx) {
...
export function $bindingElementList(nodes, parent, ctx) {
    const len = nodes.length;
    const $nodes = Array(len);
    for (let i = 0; i < len; ++i) {
...
export class $SpreadElement {
    ...
    AccumulateArray(ctx, array, nextIndex) {
        ...
        while (true) {
            ...
            const status = $CreateDataProperty(ctx, array, nextIndex.ToUint32(ctx).ToString(ctx), nextValue);
