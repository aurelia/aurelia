...
import { $AbstractRelationalComparison, $InstanceOfOperator, $AbstractEqualityComparison, $StrictEqualityComparison, $Call, $Construct, $CreateDataProperty, $Set, $CopyDataProperties, } from '../operations';
...
import { $IteratorStep, $IteratorValue, } from '../globals/iteration';
import { $TypeError, $ReferenceError, } from '../types/error';
import { $ArrayExoticObject, } from '../exotics/array';
import { $List, } from '../types/list';
import { modifiersToModifierFlags, hasBit, $identifier, $$propertyName, $assignmentExpression, $LHSExpression, IsConstructor, $unaryExpression, $i, } from './_shared';
...
export function $argumentOrArrayLiteralElement(node, parent, ctx, idx) {
    switch (node.kind) {
...
export function $argumentOrArrayLiteralElementList(nodes, parent, ctx) {
        ...
        for (let i = 0, ii = elements.length; i < ii; ++i) {
            ...
            switch (el.$kind) {
                ...
                default: {
                    ...
                    const created = $CreateDataProperty(ctx, array, new $Number(realm, postIndex['[[Value]]'] + padding).ToUint32(ctx).ToString(ctx), initValue);
...
export function $$objectLiteralElementLikeList(nodes, parent, ctx) {
    if (nodes === void 0 || nodes.length === 0) {
        return emptyArray;
    }
    const len = nodes.length;
...
export function $EvaluateCall(ctx, func, ref, $arguments, tailPosition) {
    ctx.checkTimeout();
    const realm = ctx.Realm;
    const intrinsics = realm['[[Intrinsics]]'];
    let thisValue;
...
export function $ArgumentListEvaluation(ctx, args) {
    ctx.checkTimeout();
    // Arguments : ( )
...
export function $$templateSpanList(nodes, parent, ctx) {
    if (nodes.length === 0) {
        return emptyArray;
    }
    const len = nodes.length;
