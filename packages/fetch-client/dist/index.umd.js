(function (global, factory) {
  ...
  (global = global || self, factory(global.fetchClient = {}, global.kernel, global.runtime));
          ...
          return Promise.resolve().then(() => {
              if (retryConfig.counter < retryConfig.maxRetries) {
                  ...
                  return Promise.resolve(result).then(doRetry => {
                      if (doRetry) {
                          retryConfig.counter++;
                          return new Promise(resolve => kernel.PLATFORM.global.setTimeout(resolve, calculateDelay(retryConfig) || 0))
                              .then(() => {
  ...
  class HttpClient {
      ...
      trackRequestStart() {
          ...
          if (this.isRequesting) {
              const evt = runtime.DOM.createCustomEvent('aurelia-fetch-client-request-started', { bubbles: true, cancelable: true });
              kernel.PLATFORM.setTimeout(() => runtime.DOM.dispatchEvent(evt), 1);
          }
      }
      trackRequestEnd() {
          this.isRequesting = !!(--this.activeRequestCount);
          if (!this.isRequesting) {
              const evt = runtime.DOM.createCustomEvent('aurelia-fetch-client-requests-drained', { bubbles: true, cancelable: true });
              kernel.PLATFORM.setTimeout(() => runtime.DOM.dispatchEvent(evt), 1);
          }
      }
