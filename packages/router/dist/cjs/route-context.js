...
exports.$RecognizedRoute = exports.RouteContext = exports.IRouteContext = void 0;
...
const viewport_agent_js_1 = require("./viewport-agent.js");
const component_agent_js_1 = require("./component-agent.js");
...
class RouteContext {
    constructor(viewportAgent, parent, component, definition, parentContainer) {
        ...
        this.logger = parentContainer.get(kernel_1.ILogger).scopeTo(`RouteContext<${this.friendlyPath}>`);
        this.logger.trace('constructor()');
        this.moduleLoader = parentContainer.get(kernel_1.IModuleLoader);
        const container = this.container = parentContainer.createChild();
        container.registerResolver(runtime_html_1.IController, this.hostControllerProvider = new kernel_1.InstanceProvider(), true);
        // We don't need to store it here but we use an InstanceProvider so that it can be disposed indirectly via the container.
        const contextProvider = new kernel_1.InstanceProvider();
        container.registerResolver(exports.IRouteContext, contextProvider, true);
        contextProvider.prepare(this);
        container.register(definition);
        container.register(...component.dependencies);
        this.recognizer = new route_recognizer_1.RouteRecognizer();
        const promises = [];
        const allPromises = [];
        for (const child of definition.config.routes) {
            if (child instanceof Promise) {
                ...
                const routeDef = route_definition_js_1.RouteDefinition.resolve(child, this);
                if (routeDef instanceof Promise) {
                    if (validation_js_1.isPartialChildRouteConfig(child) && child.path != null) {
                        for (const path of util_js_1.ensureArrayOfStrings(child.path)) {
                        ...
                        allPromises.push(p.then(kernel_1.noop));
                    }
                    else {
    ...
    static setRoot(container) {
        const logger = container.get(kernel_1.ILogger).scopeTo('RouteContext');
        if (!container.has(runtime_html_1.IAppRoot, true)) {
        ...
        if (container.has(exports.IRouteContext, true)) {
        ...
        const { controller } = container.get(runtime_html_1.IAppRoot);
        ...
        const router = container.get(router_js_1.IRouter);
        const routeContext = router.getRouteContext(null, controller.context.definition, controller.context);
        container.register(kernel_1.Registration.instance(exports.IRouteContext, routeContext));
        routeContext.node = router.routeTree.root;
    }
    static resolve(root, context) {
        const rootContainer = root.container;
        const logger = rootContainer.get(kernel_1.ILogger).scopeTo('RouteContext');
        ...
        if (context instanceof rootContainer.get(runtime_html_1.IPlatform).Node) {
            try {
                ...
                const controller = runtime_html_1.CustomElement.for(context, { searchParents: true });
                ...
                return controller.container.get(exports.IRouteContext);
            }
            catch (err) {
        ...
        if (runtime_html_1.isCustomElementViewModel(context)) {
        ...
        if (runtime_html_1.isCustomElementController(context)) {
    ...
    createComponentAgent(hostController, routeNode) {
        ...
        const routeDefinition = route_definition_js_1.RouteDefinition.resolve(routeNode.component);
        const componentInstance = this.container.get(routeDefinition.component.key);
        const componentAgent = component_agent_js_1.ComponentAgent.for(componentInstance, hostController, routeNode, this);
        this.hostControllerProvider.dispose();
        return componentAgent;
    }
    registerViewport(viewport) {
        const agent = viewport_agent_js_1.ViewportAgent.for(viewport, this);
    ...
    addRoute(routeable) {
        ...
        return kernel_1.onResolve(route_definition_js_1.RouteDefinition.resolve(routeable, this), routeDef => {
            for (const path of routeDef.path) {
                this.$addRoute(path, routeDef.caseSensitive, routeDef);
            }
            this.childRoutes.push(routeDef);
...
function isCustomElementDefinition(value) {
    return runtime_html_1.CustomElement.isType(value.Type);
}
