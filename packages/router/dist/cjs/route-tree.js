...
function updateNode(log, vit, ctx, node) {
    ...
    return kernel_1.onResolve(maybePromise, () => {
        if (node.context === ctx) {
            ...
            node.clearChildren();
            return kernel_1.resolveAll(...vit.children.map(vi => {
...
function processResidue(node) {
    ...
    return kernel_1.onResolve(ctx.resolved, () => {
        return kernel_1.resolveAll(...node.residue.splice(0).map(vi => {
            ...
            const defaultInstruction = instructions_js_1.ViewportInstruction.create(vpa.viewport.default);
...
function createNode(log, node, vi, append) {
    ...
    if (rr === null) {
        ...
        const ced = ctx.find(runtime_html_1.CustomElement, name);
