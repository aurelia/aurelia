...
exports.createAndAppendNodes = exports.getDynamicChildren = exports.processResidue = exports.updateRouteTree = exports.RouteTree = exports.RouteNode = void 0;
...
class RouteTree {
    ...
    finalizeInstructions() {
        return new instructions_js_1.ViewportInstructionTree(this.options, true, this.root.children.map(x => x.finalizeInstruction()), this.queryParams, this.fragment);
    }
    toString() {
...
exports.updateRouteTree = updateRouteTree;
function updateNode(log, vit, ctx, node) {
    ...
    return kernel_1.onResolve(maybePromise, () => {
        if (node.context === ctx) {
            // Do an in-place update (remove children and re-add them by compiling the instructions into nodes)
            node.clearChildren();
            return kernel_1.onResolve(kernel_1.resolveAll(...vit.children.map(vi => {
                return createAndAppendNodes(log, node, vi, node.tree.options.append || vi.append);
            })), () => {
                return kernel_1.resolveAll(...ctx.getAvailableViewportAgents('dynamic').map(vpa => {
                    const defaultInstruction = instructions_js_1.ViewportInstruction.create({
        ...
        return kernel_1.resolveAll(...node.children.map(child => {
...
function processResidue(node) {
    ...
    return kernel_1.onResolve(ctx.resolved, () => {
        return kernel_1.resolveAll(...node.residue.splice(0).map(vi => {
...
function getDynamicChildren(node) {
    ...
    return kernel_1.onResolve(ctx.resolved, () => {
        ...
        return kernel_1.onResolve(kernel_1.resolveAll(...node.residue.splice(0).map(vi => {
            return createAndAppendNodes(log, node, vi, node.append);
...
exports.getDynamicChildren = getDynamicChildren;
    ...
    switch (vi.component.type) {
        case 0 /* string */: {
            switch (vi.component.value) {
                ...
                // falls through
                case '.':
                    return kernel_1.resolveAll(...vi.children.map(childVI => {
        ...
        case 2 /* CustomElementDefinition */: {
            const routeDef = route_definition_js_1.RouteDefinition.resolve(vi.component.value);
            const childNode = createDirectNode(log, node, vi, append, routeDef.component);
            return appendNode(log, node, childNode);
        }
    }
}
exports.createAndAppendNodes = createAndAppendNodes;
...
function createConfiguredNode(log, node, vi, append, rr, route = rr.route.endpoint.route) {
    const ctx = node.context;
    const rt = node.tree;
    return kernel_1.onResolve(route.handler, $handler => {
        route.handler = $handler;
        if ($handler.redirectTo === null) {
            const vpName = $handler.viewport;
            const ced = $handler.component;
            const vpa = ctx.resolveViewportAgent(viewport_agent_js_1.ViewportRequest.create({
                viewportName: vpName,
            ...
            childCtx.node = RouteNode.create({
                ...
                residue: rr.residue === null ? [] : [instructions_js_1.ViewportInstruction.create(rr.residue)],
            });
        ...
        const origPath = route_expression_js_1.RouteExpression.parse(route.path, false);
        const redirPath = route_expression_js_1.RouteExpression.parse($handler.redirectTo, false);
        let origCur;
        let redirCur;
        const newSegs = [];
...
function appendNode(log, node, childNode) {
    return kernel_1.onResolve(childNode, $childNode => {
