...
class NavRoute {
    ...
    _active() {
        const components = this.link.split(this.nav.router.separators.add);
...
class Viewport {
    ...
    loadComponent(component) {
        this.nextComponent = this.router.container.get(CustomElementResource.keyFrom(component.description.name));
    }
...
class Router {
    ...
    replacePaths(instruction) {
        ...
        this.historyBrowser.replacePath(viewportStates.join(this.separators.sibling) + query, fullViewportStates.join(this.separators.sibling) + query, instruction);
    }
}
...
export { HistoryBrowser, LinkHandler, Nav, Router, Scope, Viewport, ViewportCustomElement, NavCustomElement };
