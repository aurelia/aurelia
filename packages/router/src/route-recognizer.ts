...
class Candidate {
  ...
  public advance(ch: string): void {
    ...
    function $process(
      nextState: AnyState,
      skippedState: DynamicState | null,
    ): void {
...
export class RouteRecognizer {
  ...
  private readonly cache: Map<string, RecognizedRoute | null> = new Map<string, RecognizedRoute | null>();

  public constructor(config: readonly RouteDefinition[]) {
...
type SegmentToState<S> = (
  S extends StaticSegment ? StaticState :
  S extends DynamicSegment ? DynamicState :
  S extends StarSegment ? StarState :
  S extends null ? SeparatorState :
  never
);
...
class State {
  ...
  public append<S extends AnySegment | null>(segment: S, value: string): SegmentToState<S> {
    ...
    if (state === void 0) {
      nextStates.push(state = new State(this as AnyState, segment, value) as AnyState);
    }
