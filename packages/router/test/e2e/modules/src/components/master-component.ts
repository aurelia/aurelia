...
export class MasterComponent {
  ...
  static viewport = 'right';

  constructor(private router: Router, private appState: AppState) {
    this.router.addRoute({ name: 'inbox', path: '/inbox', viewports: { 'detail': ContentComponent } });
    this.router.addRoute({ name: 'message', path: '/message', viewports: { 'detail': Content2Component } });
  }
