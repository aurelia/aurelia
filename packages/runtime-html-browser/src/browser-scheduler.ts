...
function createMicrotaskFlushRequestor(wnd: Window, flush: () => void) {
  ...
  if (
    typeof wnd.queueMicrotask === 'function' &&
    isNativeFunction(wnd.queueMicrotask)
  ) {
    return function () {
      wnd.queueMicrotask(callFlush);
    };
  } else if (isNativeFunction(Promise)) {
    ...
    const observer = new (wnd as unknown as { MutationObserver: typeof MutationObserver }).MutationObserver(callFlush);
    const node = wnd.document.createTextNode('a');
