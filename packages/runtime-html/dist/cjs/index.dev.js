...
const baseName = kernel.Protocol.annotation.keyFor('bindable');
const Bindable = {
    name: baseName,
    keyFrom(name) {
    ...
    for(Type) {
        ...
        const builder = {
            add(configOrProp) {
                ...
                if (!kernel.Metadata.hasOwn(baseName, Type, prop)) {
                    kernel.Protocol.annotation.appendTo(Type, Bindable.keyFrom(prop));
                }
                kernel.Metadata.define(baseName, def, Type, prop);
                return builder;
            },
            mode(mode) {
                def.mode = mode;
        ...
        for (; ii > i; ++i) {
            ...
            if (bindingCommand === null) {
                ...
                if (expr != null) {
                    ...
                    context._attrMapper.map(el, realAttrTarget)) !== null && _a !== void 0 ? _a : kernel.camelCase(realAttrTarget)));
                }
                else {
                    switch (attrName) {
                        case 'class':
        ...
        for (; ii > i; ++i) {
            ...
            if (bindingCommand === null) {
                ...
                if (expr != null) {
                    ...
                    //      innerhtml -> innerHTML
                    //      minlength -> minLength etc...
                    context._attrMapper.map(el, realAttrTarget)) !== null && _c !== void 0 ? _c : kernel.camelCase(realAttrTarget)));
                }
                // if not a custom attribute + no binding command + not a bindable + not an interpolation
