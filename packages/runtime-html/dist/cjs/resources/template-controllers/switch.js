...
let Switch = class Switch {
    ...
    swap(initiator, flags, value) {
        ...
        return kernel_1.onResolve(this.activeCases.length > 0
    ...
    activateCases(initiator, flags) {
        ...
        return kernel_1.resolveAll(...cases.map(($case) => $case.activate(initiator, flags, scope, hostScope)));
    }
    clearActiveCases(initiator, flags, newActiveCases = []) {
        ...
        return kernel_1.onResolve(kernel_1.resolveAll(...cases.reduce((acc, $case) => {
    ...
    queue(action) {
        ...
        let promise = void 0;
        promise = this.promise = kernel_1.onResolve(kernel_1.onResolve(previousPromise, action), () => {
