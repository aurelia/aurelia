...
class TemplateBinder {
    ...
    bind(node) {
        ...
        while (i < attributes.length) {
            ...
            if (bindingCommand === null || (bindingCommand.bindingType & 4096 /* IgnoreAttr */) === 0) {
                attrInfo = semantic_model_js_1.AttrInfo.from(this.container.find(custom_attribute_js_1.CustomAttribute, attrSyntax.target), attrSyntax.target);
                if (attrInfo === null) {
    ...
    bindManifest(parentManifest, node, surrogate, manifest, manifestRoot, parentManifestRoot) {
        ...
        const definition = this.container.find(custom_element_js_1.CustomElement, name);
        const elementInfo = semantic_model_js_1.ElementInfo.from(definition, name);
