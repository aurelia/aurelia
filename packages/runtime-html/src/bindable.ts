...
export const Bindable = objectFreeze({
  ...
  getAll(Type: Constructable): readonly BindableDefinition[] {
    ...
    while (--iProto >= 0) {
      ...
      const bindableMetadata = Class[Symbol.metadata]?.[baseName] as Record<PropertyKey, BindableDefinition> | undefined;
...
const Coercer = {
  ...
  define(target: Constructable<unknown>, property: string | symbol) {
    ...
    defineMetadata(((target as any)[property] as InterceptorFunc).bind(target), target, null, Coercer.key);
  },
  for(target: Constructable<unknown>) {
    return getOwnMetadata<InterceptorFunc>(Coercer.key, target);
  }
};
