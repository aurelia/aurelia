...
import { Hooks, ICustomAttribute, ICustomElement, IEncapsulationSource, IView, LifecycleFlags, State } from '@aurelia/runtime';
...
export function $attachElement(this: Writable<ICustomElement>, flags: LifecycleFlags, encapsulationSource?: IEncapsulationSource): void {
  ...
  while (current !== null) {
    ...
    current = (current as any).$nextAttach;
  }
...
export function $detachElement(this: Writable<ICustomElement>, flags: LifecycleFlags): void {
  if (this.$state & State.isAttached) {
    ...
    while (current !== null) {
      ...
      current = (current as any).$prevAttach;
    }
