...
import { IEventManager } from '@aurelia/runtime';
import { BindingType, IExpressionParser } from '@aurelia/runtime';
import { InterpolationBinding, MultiInterpolationBinding } from '@aurelia/runtime';
import { LetBinding } from '@aurelia/runtime';
import { Listener } from '@aurelia/runtime';
import { IObserverLocator } from '@aurelia/runtime';
import { Ref } from '@aurelia/runtime';
import { ICallBindingInstruction, IHydrateAttributeInstruction, IHydrateElementInstruction, IHydrateTemplateController, IInterpolationInstruction, IIteratorBindingInstruction, ILetElementInstruction, IListenerBindingInstruction, IPropertyBindingInstruction, IRefBindingInstruction, ISetAttributeInstruction, ISetPropertyInstruction, IStylePropertyBindingInstruction, ITextBindingInstruction, TargetedInstructionType, TemplatePartDefinitions } from '@aurelia/runtime';
import { DOM, INode, IRemovableNode } from '@aurelia/runtime';
import { IAttach, IAttachables, IBindables, IBindScope, IRenderable, IRenderContext } from '@aurelia/runtime';
import { ICustomAttribute } from '@aurelia/runtime';
import { ICustomElement } from '@aurelia/runtime';
import { IInstructionRenderer, instructionRenderer, IRenderer, IRenderingEngine } from '@aurelia/runtime';
import { customElementKey, customAttributeKey } from './definition';
...
export function addBindable(renderable: IBindables, bindable: IBindScope): void {
  (bindable as any).$prevBind = renderable.$bindableTail;
  (bindable as any).$nextBind = null;
  if (renderable.$bindableTail === null) {
    renderable.$bindableHead = bindable;
  } else {
    (renderable.$bindableTail as any).$nextBind = bindable;
  }
  renderable.$bindableTail = bindable;
}

export function addAttachable(renderable: IAttachables, attachable: IAttach): void {
  (attachable as any).$prevAttach = renderable.$attachableTail;
  (attachable as any).$nextAttach = null;
  if (renderable.$attachableTail === null) {
    renderable.$attachableHead = attachable;
  } else {
    (renderable.$attachableTail as any).$nextAttach = attachable;
  }
