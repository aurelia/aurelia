...
function parseArrayLiteralExpression(expressionType: ExpressionType): ArrayBindingPattern | ArrayLiteralExpression {
  ...
  while ($currentToken !== Token.CloseBracket) {
    if (consumeOpt(Token.Comma)) {
      ...
      elements.push(parse(Precedence.Assign, expressionType === ExpressionType.IsIterator ? ExpressionType.None : expressionType) as IsAssign);
      if (consumeOpt(Token.Comma)) {
        if (($currentToken as Token) === Token.CloseBracket) {
          break;
        }
...
function parseObjectLiteralExpression(expressionType: ExpressionType): ObjectBindingPattern | ObjectLiteralExpression {
  ...
  while ($currentToken !== Token.CloseBrace) {
    ...
    if ($currentToken & Token.StringOrNumericLiteral) {
      nextToken();
      consume(Token.Colon);
      values.push(parse(Precedence.Assign, expressionType === ExpressionType.IsIterator ? ExpressionType.None : expressionType /*  & ~ExpressionType.IsIterator */) as IsAssign);
      ...
      if (consumeOpt(Token.Colon)) {
        ...
        $index = index;
        values.push(parse(Precedence.Primary, expressionType === ExpressionType.IsIterator ? ExpressionType.None : expressionType/*  & ~ExpressionType.IsIterator */) as IsAssign);
      }
    } else {
