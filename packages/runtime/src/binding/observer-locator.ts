...
import { LibUiEntryObserver, LibUiCheckedObserver } from './libui-entry-observer';
...
export class ObserverLocator implements IObserverLocator {
  ...
  private createPropertyObserver(obj: IObservable, propertyName: string): AccessorOrObserver {
    ...
    let isLibUiNode: boolean;
    if (LibUiDOM.isNodeInstance(obj)) {
      if (obj.nodeName === 'ui-entry' && propertyName === 'text') {
        return new LibUiEntryObserver(this.lifecycle, obj as UiEntry);
      }
      if (obj.nodeName === 'ui-checkbox' && propertyName === 'checked') {
        return new LibUiCheckedObserver(this.lifecycle, obj as UiCheckbox, this);
      }
    }
