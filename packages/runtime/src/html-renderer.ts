...
import { ILibUiNode, ILibUiRenderLocation, LibUiDOM } from './libui-dom';
...
export class ListenerBindingRenderer implements IInstructionRenderer {
  ...
  public render(context: IRenderContext, renderable: IRenderable, target: ILibUiNode, instruction: IListenerBindingInstruction): void {
...
export class CustomElementRenderer implements IInstructionRenderer {
  ...
  public render(context: IRenderContext, renderable: IRenderable, target: ILibUiNode, instruction: IHydrateElementInstruction): void {
    const operation = context.beginComponentOperation(renderable, target, instruction, null, null, target as ILibUiRenderLocation, true);
...
export class CustomAttributeRenderer implements IInstructionRenderer {
  ...
  public render(context: IRenderContext, renderable: IRenderable, target: ILibUiNode, instruction: IHydrateAttributeInstruction): void {
...
export class TemplateControllerRenderer implements IInstructionRenderer {
  ...
  public render(context: IRenderContext, renderable: IRenderable, target: ILibUiNode, instruction: IHydrateTemplateController, parts?: TemplatePartDefinitions): void {
    const factory = this.renderingEngine.getViewFactory(instruction.def, context);
    const operation = context.beginComponentOperation(renderable, target, instruction, factory, parts, LibUiDOM.convertToRenderLocation(target), false);
    const component = context.get<ICustomAttribute>(customAttributeKey(instruction.res));
    const instructionRenderers = context.get(IRenderer).instructionRenderers;
    const childInstructions = instruction.instructions;
