...
function isRenderLocation(node: IKonvaNode): node is IKonvaRenderLocation {
  return node instanceof konva.Node && node.nodeName === '#comment';
}

const KonvaDomMap: Record<string, (node?: Element) => IKonvaNode> = {};
export const KonvaDOM = new class {
  registerElementResolver(container: IContainer, resolver: IResolver): void {
    container.registerResolver(IKonvaNode, resolver);
  ...
  createDocumentFragment(markupOrNode?: any): DocumentFragment {
    ...
    return KonvaDOM.createTemplate(<string>markupOrNode).content;
  }
  ...
  convertToRenderLocation(node: any) {
    ...
    const locationEnd = KonvaDOM.createComment('au-end') as IKonvaRenderLocation;
    const locationStart = KonvaDOM.createComment('au-start') as IKonvaRenderLocation;
    KonvaDOM.replaceNode(locationEnd, node);
    KonvaDOM.insertBefore(locationStart, locationEnd);
  ...
  */
  createComment(text: string): IKonvaNode {
    const dObj: IKonvaNode = new konva.Node({});
    dObj.nodeName = '#comment';
    dObj.textContent = text;
    return dObj;
  }
  createTextNode(text: string): IKonvaNode {
    const textNode: IKonvaNode = new konva.Text({ text });
  ...
  createElement(tagName: string, node?: Element): IKonvaNode;
  createElement(tagName: string, node?: Element): IKonvaNode {
    if (!(tagName in KonvaDomMap)) {
      console.log(Object.keys(KonvaDomMap));
      throw new Error('There is no element with ' + tagName + ' registered');
    }
    let blessedNode: IKonvaNode = KonvaDomMap[tagName](node);
    blessedNode.nodeName = tagName;
    return blessedNode;
  }
  remove<T extends IKonvaNode = IKonvaNode>(childView: T, parentView?: IKonvaNode): T | null {
    // // if (node.parent) {
  ...
  replaceNode(newNode: IKonvaNode, refNode: IKonvaNode) {
    KonvaDOM.insertBefore(newNode, refNode);
    KonvaDOM.remove(refNode);
    // refNode.parent.insertBefore(newNode, refNode);
    // refNode.parent.removeChild(refNode);
    return newNode;
  }
  insertBefore<T extends IKonvaNode = IKonvaNode>(newNode: T, refNode: IKonvaNode): T {
  ...
  appendChild<T extends IKonvaNode = IKonvaNode>(node: T, parentNode: IKonvaNode): T {
    if (parentNode instanceof konva.Container) {
      parentNode.add(node);
    } else {
  ...
  isNodeInstance(node: any): node is IKonvaNode {
    return node instanceof konva.Node;
  }
  map(tagName: string, ctor: (() => IKonvaNode)): void {
    if (tagName in KonvaDomMap) {
      throw new Error(`Element with the same name "${tagName}" already exists`);
    }
    KonvaDomMap[tagName] = ctor;
  }
  nodesToNsViews = nodeToKonvaNodes;

  treatAsNonWhitespace(node: IKonvaNode): void {
    // see isAllWhitespace above
    (<any>node).auInterpolationTarget = true;
  }
};

export const IKonvaNode = DI.createInterface<IKonvaNode>().noDefault();
export interface IKonvaNode extends konva.Node {
  nodeType?: number;
  nodeName?: string;
  textContent?: string;
  firstChild?: IKonvaNode | null;
  lastChild?: IKonvaNode | null;
  nextSibling?: IKonvaNode | null;
  previousSibling?: IKonvaNode | null;
}
export const IKonvaRenderLocation = DI.createInterface<IKonvaRenderLocation>().noDefault();
export interface IKonvaRenderLocation extends IKonvaNode {
  $start: IKonvaRenderLocation;
  $nodes: IKonvaNodeSequence;
}
/**
* Represents a DocumentFragment
*/
export interface IKonvaNodeSequence {
  firstChild: IKonvaNode;
  lastChild: IKonvaNode;
  /**
   * The nodes of this sequence.
   */
  children: ReadonlyArray<IKonvaNode>;
  /**
  * Find all instruction targets in this sequence.
  */
  findTargets(): ReadonlyArray<IKonvaNode>;
  /**
  * Insert this sequence as a sibling before refNode
  */
  insertBefore(refNode: IKonvaNode): void;
  /**
  * Append this sequence as a child to parent
  */
  appendTo(parent: IKonvaNode): void;
  /**
...
const emptySequence: IKonvaNodeSequence = {
  firstChild: null,
  lastChild: null,
  children: PLATFORM.emptyArray,
  findTargets(): ReturnType<IKonvaNodeSequence['findTargets']> { return PLATFORM.emptyArray; },
  insertBefore(refNode: IKonvaNode): ReturnType<IKonvaNodeSequence['insertBefore']> { /*do nothing*/ },
  appendTo(parent: IKonvaNode): ReturnType<IKonvaNodeSequence['appendTo']> { /*do nothing*/ },
  remove(): ReturnType<IKonvaNodeSequence['remove']> { /*do nothing*/ }
};
...
*/
export class KonvaTextNodeSequence implements IKonvaNodeSequence {
  public firstChild: IKonvaNode;
  public lastChild: IKonvaNode;
  public children: IKonvaNode[];
  private targets: ReadonlyArray<IKonvaNode>;
  constructor(text: IKonvaNode) {
  ...
  public findTargets(): ReadonlyArray<IKonvaNode> {
    return this.targets;
  }
  public insertBefore(refNode: IKonvaNode): void {
    KonvaDOM.insertBefore(this.firstChild, refNode);
    // refNode.parent.insertBefore(this.firstChild, refNode);
  }
  public appendTo(parent: IKonvaNode): void {
...
/*@internal*/
export class KonvaFragmentNodeSequence implements IKonvaNodeSequence {
  public firstChild: IKonvaNode;
  public lastChild: IKonvaNode;
  public children: ReadonlyArray<IKonvaNode>;
  ...
  constructor(fragment: DocumentFragment) {
    ...
    const targetNodeList: IKonvaNode[] = [];
    const nsNodes = this.children = nodeToKonvaNodes(fragment.childNodes, null, targetNodeList);
    ...
    while (i < ii) {
      ...
      if (target.nodeName === 'au-marker') {
        ...
        targets[i] = KonvaDOM.convertToRenderLocation(target);
      } else {
  ...
  public insertBefore(refNode: IKonvaNode): void {
    ...
    while (children.length > 0) {
      KonvaDOM.insertBefore(children[0], refNode);
    }
  ...
  public appendTo(parent: IKonvaNode): void {
    ...
    // }
    this.children.forEach(c => KonvaDOM.appendChild(c, parent));
...
export interface IKonvaNodeSequenceFactory {
  createNodeSequence(): IKonvaNodeSequence;
}
export class KonvaNodeSequenceFactory {
  ...
  constructor(fragment: DocumentFragment) {
    ...
    switch (childNodes.length) {
      ...
      case 2:
        ...
        if (target.nodeName === 'AU-MARKER' || target.nodeName === '#comment') {
          ...
          if (text.nodeType === NodeTypes.TEXT && text.textContent === ' ') {
            ...
            this.Type = KonvaTextNodeSequence;
            return;
          }
        }
      ...
      default:
        this.deepClone = true;
        this.node = fragment;
        this.Type = KonvaFragmentNodeSequence;
    }
  }
  public static createFor(markupOrNode: string | INode): KonvaNodeSequenceFactory {
    const fragment = KonvaDOM.createDocumentFragment(markupOrNode);
    return new KonvaNodeSequenceFactory(fragment);
  }
  public createNodeSequence(): IKonvaNodeSequence {
...
function nodeToKonvaNodes(nodes: Node[] | ArrayLike<Node>, parent: IKonvaNode = null, targets: IKonvaNode[] = []): IKonvaNode[] {
  const results: IKonvaNode[] = [];
  for (let i = 0, ii = nodes.length; ii > i; ++i) {
    const node = nodes[i];
    let konvaNode: IKonvaNode | null = null;
    if (!node.nodeType) {
    ...
    switch (node.nodeType) {
      case NodeTypes.ELEMENT:
        ...
        konvaNode = KonvaDOM.createElement(nodeName, node as Element);
    ...
    if (parent !== null) {
      KonvaDOM.appendChild(konvaNode, parent);
      // parent.addChild(pixiElement);
    }
    if (node.childNodes.length > 0) {
      // if (nsView instanceof PIXI.Container) {
      nodeToKonvaNodes(node.childNodes, konvaNode, targets);
      // } else {
      //   throw new Error(
