...
import { ILibUiNode, ILibUiRenderLocation, ILibUiNodeSequence } from './libui-dom';
...
export interface IRenderable extends IBindables, IAttachables, IState {
   ...
   */
  readonly $nodes: ILibUiNodeSequence;
...
export interface IRenderContext extends IServiceLocator {
  ...
  render(renderable: IRenderable, targets: ArrayLike<ILibUiNode>, templateDefinition: TemplateDefinition, host?: ILibUiNode, parts?: TemplatePartDefinitions): void;
  beginComponentOperation(renderable: IRenderable, target: ILibUiNode, instruction: Immutable<ITargetedInstruction>, factory?: IViewFactory, parts?: TemplatePartDefinitions, location?: ILibUiRenderLocation, locationIsContainer?: boolean): IDisposable;
}

export interface IView extends IBindScope, IRenderable, IAttach, IMountable {
  readonly cache: IViewCache;
  readonly isFree: boolean;
  readonly location: ILibUiRenderLocation;

  hold(location: ILibUiRenderLocation, flags: LifecycleFlags): void;
