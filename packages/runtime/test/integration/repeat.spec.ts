...
//   const sut = new Repeat<T>(lifecycle, location, renderable, factory);
...
//         this.lifecycle.beginAttach(lifecycle, host, LifecycleFlags.none).attach(sut).end();
...
//         this.lifecycle.beginDetach(lifecycle, LifecycleFlags.none).detach(sut).end();
...
//       ([items, count], $2, $3, [newItems, newCount]) => [(sut, host, lifecycle) => {
//         this.lifecycle.beginDetach(lifecycle, LifecycleFlags.unbindAfterDetached).detach(sut).end();
