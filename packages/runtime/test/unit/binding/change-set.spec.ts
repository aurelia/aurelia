...
import { IChangeTracker, ChangeSet, IChangeSet, LinkedChangeList } from '../../../src/index';
...
describe('LinkedChangeList', () => {
  let sut: LinkedChangeList;

  beforeEach(() => {
    sut = new LinkedChangeList();
  });

  it('creates its own promise per instance', () => {
    const sut2 = new LinkedChangeList();
  ...
  describe('add()', () => {
    for (const changeTrackers of createChangeTrackersArr()) {
      it(`adds ${changeTrackers.length} ChangeTracker(s) to the set`, () => {
        for (const changeTracker of changeTrackers) {
          sut.add(changeTracker);
          expect(sut['tail'] === changeTracker).to.be.true;
        }
      });
    }
