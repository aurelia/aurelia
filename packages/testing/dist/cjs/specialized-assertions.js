...
function nextNode(host, node) {
    return runtime_html_1.CustomElement.for(node, { optional: true })?.shadowRoot?.firstChild ?? node.firstChild ?? node.nextSibling ?? nextAncestor(host, node);
}
function getVisibleText(host, removeWhiteSpace) {
    let text = '';
    let cur = runtime_html_1.CustomElement.for(host, { optional: true })?.shadowRoot?.firstChild ?? host.firstChild;
    while (cur !== null) {
