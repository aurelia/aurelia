(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@aurelia/kernel'), require('@aurelia/debug'), require('@aurelia/runtime-html'), require('@aurelia/runtime'), require('@aurelia/jit')) :
  typeof define === 'function' && define.amd ? define(['exports', '@aurelia/kernel', '@aurelia/debug', '@aurelia/runtime-html', '@aurelia/runtime', '@aurelia/jit'], factory) :
  (global = global || self, factory(global.testing = {}, global.kernel, global.debug, global.runtimeHtml, global.runtime, global.jit));
}(this, function (exports, kernel, debug, runtimeHtml, runtime, jit) { 'use strict';
  ...
  const BigInt_valueOf = uncurryThis(BigInt.prototype.valueOf);
  ...
  function getOwnNonIndexProperties(val, showHidden) {
      if (showHidden) {
          return getOwnPropertyNames(val).filter(k => !kernel.isNumeric(k));
      }
      else {
          return Object_keys(val).filter(k => !kernel.isNumeric(k));
      }
  }
  ...
  const RuntimeTracer = { ...kernel.Tracer };
  function enableTracing() {
      Object.assign(kernel.Tracer, debug.Tracer);
      kernel.Tracer.enabled = true;
  }
  function disableTracing() {
      kernel.Tracer.flushAll(null);
      Object.assign(kernel.Tracer, RuntimeTracer);
      kernel.Tracer.enabled = false;
  }
          ...
          for (let i = 0, ii = params.length; i < ii; ++i) {
              ...
              switch (typeof p) {
                  ...
                  case 'boolean':
                      ...
                      break;
                  case 'number':
                      output += p > 0 ? `flags=${debug.stringifyLifecycleFlags(p)}` : '0';
                      break;
                  case 'object':
                      if (p === null) {
                          output += 'null';
          ...
          case 'args':
              if (value instanceof Call) {
                  switch (value.method) {
                      ...
                      case 'unbinding':
                      case 'unbound':
                          value.args[0] = debug.stringifyLifecycleFlags(value.args[0]);
                          break;
                      case 'valueChanged':
                          value.args[2] = debug.stringifyLifecycleFlags(value.args[2]);
                          break;
                      case 'swap':
                      case 'updateView':
                          value.args[1] = debug.stringifyLifecycleFlags(value.args[1]);
                          break;
                  }
              }
              break;
  ...
  function formatRaw(ctx, value, recurseTimes, typedArray) {
      ...
      try {
          ...
          let $key;
          const isNotNode = !(value instanceof runtimeHtml.DOM.Node);
          for (i = 0; i < keys.length; i++) {
              $key = keys[i];
              if (
              // Aurelia-specific:
  ...
  function verifyASTEqual(actual, expected, errors, path) {
      if (expected == null) {
          ...
          const expectedSerialized = debug.Serializer.serialize(expected);
          assert.strictEqual(actual, expectedSerialized, `actual`);
      }
      else {
          const expectedSerialized = debug.Serializer.serialize(expected);
          const expectedUnparsed = debug.Unparser.unparse(expected);
          const actualSerialized = debug.Serializer.serialize(actual);
          const actualUnparsed = debug.Unparser.unparse(actual);
  ...
  function $getVisibleText(root, context) {
      ...
      for (let i = 0; i < length; ++i) {
          ...
          else if (controller.viewModel instanceof runtime.Replaceable) {
              $getVisibleText(controller.viewModel.view, context);
          }
          else if (controller.viewModel instanceof runtime.With) {
              $getVisibleText(controller.viewModel.view, context);
          }
          else if (controller.viewModel instanceof runtime.If) {
              $getVisibleText(controller.viewModel.view, context);
          }
          else if (controller.viewModel instanceof runtimeHtml.Compose) {
              $getVisibleText(controller.viewModel.view, context);
          }
          else if (controller.viewModel instanceof runtime.Repeat) {
  ...
  ***************************************************************************** */
  ...
  class AuProjector {
      ...
      project(nodes) {
          if (kernel.Tracer.enabled) {
              kernel.Tracer.enter('AuProjector', 'project', slice.call(arguments));
          }
      ...
      take(nodes) {
          if (kernel.Tracer.enabled) {
              kernel.Tracer.enter('AuProjector', 'take', slice.call(arguments));
          }
          nodes.remove();
          if (kernel.Tracer.enabled) {
              kernel.Tracer.leave();
  ...
  AuDOMInitializer.inject = [kernel.IContainer];
  class AuTemplateFactory {
      constructor(dom) {
          this.dom = dom;
      }
      create(parentRenderContext, definition) {
          return new runtime.CompiledTemplate(this.dom, definition, new AuNodeSequenceFactory(this.dom, definition.template), parentRenderContext);
      }
  }
  AuTemplateFactory.inject = [runtime.IDOM];
  ...
  class AuTextRenderer {
      ...
      render(flags, dom, context, renderable, target, instruction) {
          if (kernel.Tracer.enabled) {
              kernel.Tracer.enter('AuTextRenderer', 'render', slice.call(arguments));
          }
          let realTarget;
          ...
          const bindable = new runtime.Binding(instruction.from, realTarget, 'textContent', runtime.BindingMode.toView, this.observerLocator, context);
          runtime.addBinding(renderable, bindable);
          if (kernel.Tracer.enabled) {
              kernel.Tracer.leave();
          }
      }
  };
  exports.AuTextRenderer = __decorate([
      kernel.inject(runtime.IObserverLocator),
      runtime.instructionRenderer('au')
      /** @internal */
  ], exports.AuTextRenderer);
  const AuDOMConfiguration = {
      register(container) {
          container.register(runtime.RuntimeBasicConfiguration, exports.AuTextRenderer, kernel.Registration.singleton(runtime.IDOM, AuDOM), kernel.Registration.singleton(runtime.IDOMInitializer, AuDOMInitializer), kernel.Registration.singleton(runtime.IProjectorLocator, AuProjectorLocator), kernel.Registration.singleton(runtime.ITargetAccessorLocator, AuObserverLocator), kernel.Registration.singleton(runtime.ITargetObserverLocator, AuObserverLocator), kernel.Registration.singleton(runtime.ITemplateFactory, AuTemplateFactory));
      },
      createContainer() {
  ...
  const AuDOMTest = {
      setup() {
          ...
          const au = new runtime.Aurelia(container);
          const lifecycle = container.get(runtime.ILifecycle);
      ...
      createTextDefinition(expression, name = `${expression}-text`) {
          return {
              ...
              // @ts-ignore
              instructions: [[new AuTextInstruction(jit.parseExpression(expression))]]
          };
      },
      ...
      createIfInstruction(expression, def) {
          return new runtime.HydrateTemplateController(def, 'if', 
          // @ts-ignore
          [new runtime.ToViewBindingInstruction(jit.parseExpression(expression), 'value')]);
      },
      createElseInstruction(def) {
          return new runtime.HydrateTemplateController(def, 'else', [], true);
      },
      createRepeatInstruction(expression, def) {
          return new runtime.HydrateTemplateController(def, 'repeat', 
          // @ts-ignore
          [new runtime.IteratorBindingInstruction(jit.parseExpression(expression, 539 /* ForCommand */), 'items')]);
      },
      createReplaceableInstruction(def) {
          return new runtime.HydrateTemplateController(def, 'replaceable', []);
      },
      createWithInstruction(expression, def) {
          return new runtime.HydrateTemplateController(def, 'with', 
          // @ts-ignore
          [new runtime.ToViewBindingInstruction(jit.parseExpression(expression), 'value')]);
      },
      createElementInstruction(name, bindings, parts) {
          return new runtime.HydrateElementInstruction(name, 
          // @ts-ignore
          bindings.map(([from, to]) => new runtime.ToViewBindingInstruction(jit.parseExpression(from), to)), parts);
      },
      createLetInstruction(bindings, toViewModel = false) {
          return new runtime.LetElementInstruction(
          // @ts-ignore
          bindings.map(([from, to]) => new runtime.LetBindingInstruction(jit.parseExpression(from), to)), toViewModel);
      }
  };
  ...
  */
  ...
  const emptyArray = kernel.PLATFORM.emptyArray;
  function h(name, attrs = null, ...children) {
      const el = runtimeHtml.DOM.createElement(name);
      ...
      for (const child of children) {
          ...
          childrenCt.appendChild(isNodeOrTextOrComment(child)
              ? child
              : runtimeHtml.DOM.createTextNode(`${child}`));
      }
      return el;
  }
  ...
  const hJsx = function (name, attrs, ...children) {
      const el = runtimeHtml.DOM.createElement(name === 'let$' ? 'let' : name);
      if (attrs != null) {
          let value;
          for (const attr in attrs) {
              value = attrs[attr];
      ...
      for (const child of children) {
          ...
          if (Array.isArray(child)) {
              for (const child_child of child) {
                  appender.appendChild(runtimeHtml.DOM.isNodeInstance(child_child) ? child_child : runtimeHtml.DOM.createTextNode('' + child_child));
              }
          }
          else {
              appender.appendChild(runtimeHtml.DOM.isNodeInstance(child) ? child : runtimeHtml.DOM.createTextNode('' + child));
          }
      }
      return el;
  };
  ...
  class HTMLTestContext {
      get container() {
          if (this._container === void 0) {
              this._container = kernel.DI.createContainer(this.config);
              kernel.Registration.instance(runtime.IDOM, this.dom).register(this._container);
              kernel.Registration.instance(HTMLTestContext, this).register(this._container);
              this._container.register(debug.DebugConfiguration);
          }
          return this._container;
      }
      get templateCompiler() {
          if (this._templateCompiler == void 0) {
              this._templateCompiler = this.container.get(runtime.ITemplateCompiler);
          }
          return this._templateCompiler;
      }
      get observerLocator() {
          if (this._observerLocator === void 0) {
              this._observerLocator = this.container.get(runtime.IObserverLocator);
          }
          return this._observerLocator;
      }
      get lifecycle() {
          if (this._lifecycle === void 0) {
              this._lifecycle = this.container.get(runtime.ILifecycle);
          }
          return this._lifecycle;
      }
      get renderer() {
          if (this._renderer === void 0) {
              this._renderer = this.container.get(runtime.IRenderer);
          }
          return this._renderer;
      }
      get projectorLocator() {
          if (this._projectorLocator === void 0) {
              this._projectorLocator = this.container.get(runtime.IProjectorLocator);
          }
          return this._projectorLocator;
      }
      get renderingEngine() {
          if (this._renderingEngine === void 0) {
              this._renderingEngine = this.container.get(runtime.IRenderingEngine);
          }
      ...
      constructor(config, wnd, UIEventType, EventType, CustomEventType, NodeType, ElementType, HTMLElementType, HTMLDivElementType, TextType, CommentType, DOMParserType) {
          ...
          this.dom = new runtimeHtml.HTMLDOM(this.wnd, this.doc, NodeType, ElementType, HTMLElementType, CustomEventType);
  ...
  function writeProfilerReport(testName) {
      let msg = '\n';
      kernel.Profiler.report(function (name, duration, topLevel, total) {
  ...
  function createObserverLocator(containerOrLifecycle) {
      ...
      if (containerOrLifecycle === undefined || !('get' in containerOrLifecycle)) {
          container = kernel.DI.createContainer();
          container.register(runtime.ILifecycleRegistration);
      }
      else {
      ...
      kernel.Registration.instance(runtime.ITargetObserverLocator, dummyLocator).register(container);
      kernel.Registration.instance(runtime.ITargetAccessorLocator, dummyLocator).register(container);
      container.register(runtime.IObserverLocatorRegistration);
      return container.get(runtime.IObserverLocator);
  }
  function createScopeForTest(bindingContext = {}, parentBindingContext) {
      if (parentBindingContext) {
          return runtime.Scope.create(0 /* none */, bindingContext, runtime.OverrideContext.create(0 /* none */, bindingContext, runtime.OverrideContext.create(0 /* none */, parentBindingContext, null)));
      }
      return runtime.Scope.create(0 /* none */, bindingContext, runtime.OverrideContext.create(0 /* none */, bindingContext, null));
  }
  ...
  exports.AuDOM = AuDOM;
  exports.AuDOMConfiguration = AuDOMConfiguration;
  exports.AuDOMInitializer = AuDOMInitializer;
  exports.AuDOMTest = AuDOMTest;
  ...
  exports.SymbolTraceWriter = SymbolTraceWriter;
