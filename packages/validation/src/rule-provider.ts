...
export class PropertyRule<TObject extends IValidateable = IValidateable, TValue = unknown> implements IPropertyRule {
  ...
  public async validate(
    ...
    if (expression === void 0) {
      ...
      value = expression.evaluate(flags, scope, null, this.locator, null, null); // TODO: get proper hostScope?
    }
    ...
    const validateRuleset = async (rules: IValidationRule[]) => {
      const validateRule = async (rule: IValidationRule) => {
        ...
        if (!isValidOrPromise) {
          ...
          message = this.messageProvider.getMessage(rule).evaluate(flags!, messageEvaluationScope, null, null!, null, null) as string;
        }
