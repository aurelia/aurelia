...
import { Benchmark, BenchmarkType, LighthouseData, benchmarks } from './benchmarks'
import { BenchmarkOptions, ErrorAndWarning, FrameworkData, TConfig, config as defaultConfig } from './common'
...
async function computeResultsCPU(driver: WebDriver, _benchmarkOptions: BenchmarkOptions, framework: FrameworkData, benchmark: Benchmark, warnings: String[], expcectedResultCount: number): Promise<number[]> {
...
async function computeResultsMEM(driver: WebDriver, _benchmarkOptions: BenchmarkOptions, _framework: FrameworkData, _benchmark: Benchmark, _warnings: String[]): Promise<number> {
  let entriesBrowser = await driver.manage().logs().get(logging.Type.BROWSER);
  if (config.LOG_DEBUG) console.log("browser entries", entriesBrowser);
